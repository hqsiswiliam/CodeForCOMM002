...
-(NSArray*)retrieveAllQueryAsSentencesArray{
    ...
    for (MiddleTable *table in middleTables) {
        ...
        if (table.idOfLatitude!=[NSNumber numberWithLongLong:-1]) {
            __unused Element *e = [self fetchFor:@"Latitude" withRow_id:table.idOfLatitude];
        }
        
        if (table.idOfLongtitude!=[NSNumber numberWithLongLong:-1]) {
            __unused Element *e = [self fetchFor:@"Longtitude" withRow_id:table.idOfLongtitude];
        }
...
-(RetrieveForDisplay*)retrieveAllQueryAsRetriveForDisplay{
    @try {
        RetrieveForDisplay *display_obj = [[RetrieveForDisplay alloc]init];
        NSMutableArray *arrayToReturn = [NSMutableArray array];
        NSArray *middleTables = [self fetchAllFor:@"MiddleTable"];
        for (MiddleTable *table in middleTables) {
            NSMutableString *sentence = [NSMutableString stringWithString:@""];
            if (table.idOfVerb!=[NSNumber numberWithLongLong:-1]) {
                Element *e = [self fetchFor:@"Verb" withRow_id:table.idOfVerb];
                [sentence appendString:e.content];
                [display_obj.verbs addObject:e.content];
            }else{[display_obj.verbs addObject:@""];}
            if (table.idOfThing!=[NSNumber numberWithLongLong:-1]) {
                Element *e = [self fetchFor:@"Thing" withRow_id:table.idOfThing];
                [sentence appendFormat:@" %@",e.content];
                [display_obj.things addObject:e.content];
            }else{[display_obj.things addObject:@""];}
            if (table.idOfPerson!=[NSNumber numberWithLongLong:-1]) {
                Element *e = [self fetchFor:@"Person" withRow_id:table.idOfPerson];
                [sentence appendFormat:@" to %@",e.content];
                [display_obj.persons addObject:e.content];
            }else{[display_obj.persons addObject:@""];}
            if (table.idOfPlace!=[NSNumber numberWithLongLong:-1]) {
                Element *e = [self fetchFor:@"Place" withRow_id:table.idOfPlace];
                [sentence appendFormat:@" at %@",e.content];
                [display_obj.places addObject:e.content];
            }else{[display_obj.places addObject:@""];}
            if (table.idOfMyDate!=[NSNumber numberWithLongLong:-1]) {
                Element *e = [self fetchFor:@"MyDate" withRow_id:table.idOfMyDate];
                [sentence appendFormat:@" on %@",e.content];
                [display_obj.myDates addObject:e.content];
            }else{[display_obj.myDates addObject:@""];}
            
            if (table.idOfLatitude!=[NSNumber numberWithLongLong:-1]) {
                 Element *e = [self fetchFor:@"Latitude" withRow_id:table.idOfLatitude];
                [display_obj.latitudes addObject:e.content];
            }else{[display_obj.latitudes addObject:@""];}
            
            if (table.idOfLongtitude!=[NSNumber numberWithLongLong:-1]) {
                 Element *e = [self fetchFor:@"Longtitude" withRow_id:table.idOfLongtitude];
                [display_obj.longtitudes addObject:e.content];
            }else{[display_obj.longtitudes addObject:@""];}
            if (![sentence isEqualToString:@""]) {
                [sentence appendString:@"."];
//                NSLog(@"sentence:%@",sentence);
                [arrayToReturn addObject:sentence];
        ...
        if ([arrayToReturn count]>0) {
            display_obj.sentences = arrayToReturn;
            NSLog(@"Number should be the same:\n %lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu",
                  (unsigned long)[display_obj.sentences count],
                  (unsigned long)[display_obj.verbs count], (unsigned long)[display_obj.things count],
                  (unsigned long)[display_obj.persons count], (unsigned long)[display_obj.places count],
                  (unsigned long)[display_obj.myDates count], (unsigned long)[display_obj.latitudes count],
                  (unsigned long)[display_obj.longtitudes count]);
            return display_obj;
        }
    }
    @catch (NSException *exception) {
        NSLog(@"ERROR IN DATAOPERATION: %@", exception);
    }
    @finally {
...
//    NSNumber *latitude_id = [NSNumber numberWithLongLong:row_id_latitude];
//    NSNumber *longtitude_id = [NSNumber numberWithLongLong:row_id_longtitude];
